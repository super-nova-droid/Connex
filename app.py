...
def event_details(event_id):
    ...
    try:
        ...
        if current_user_role == 'user': 
            check_volunteer_query = "SELECT COUNT(*) FROM event_volunteers WHERE event_id = %s AND volunteer_id = %s"
...
def volunteer_for_event():
    ...
    event_id = request.form.get('event_id', type=int)
    volunteer_id = g.user['id'] # The current guest user ID
    ...
    try:
        ...
        check_query = "SELECT COUNT(*) FROM event_volunteers WHERE event_id = %s AND volunteer_id = %s"
        cursor.execute(check_query, (event_id, volunteer_id))
        if cursor.fetchone()['COUNT(*)'] > 0:
            flash("You have already volunteered for this event.", 'warning')
            return redirect(url_for('event_details', event_id=event_id))

        insert_query = "INSERT INTO event_volunteers (event_id, volunteer_id) VALUES (%s, %s)"
        cursor.execute(insert_query, (event_id, volunteer_id))
        db_connection.commit()
...
def remove_volunteer():
    ...
    try:
        ...
        delete_query = "DELETE FROM event_volunteers WHERE event_id = %s AND volunteer_id = %s"
        cursor.execute(delete_query, (event_id, volunteer_id))
        db_connection.commit()
...
def calendar():
    ...
    try:
        ...
        # Query to get events for the left sidebar list, combining user sign-ups and volunteer assignments
        # Modified: For volunteered events, explicitly use g.user['username'] as event_username
        query = f"""
